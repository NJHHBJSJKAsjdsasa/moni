import{S as T}from"./atlas_CGEzHqiP6UrmR4JlMl8bd.js";import{D as E,a as N}from"./atlas_Be-3E54fbq6i1G6LOfhmR.js";import{cp as U,cM as G,au as p,D as w,bE as f,C as u,m as y,cr as _,cN as B,cx as O,cy as S,i as W,av as M,co as z,ct as F,ck as D,cm as P,V as R,B as L}from"./atlas_BkM-4aqZ6cV-vWVlt631_.js";const c={RING_COUNT:{min:3,max:8},RING_DISTANCE:{min:1.5,max:3.5},WAVE_COUNT:{min:51,max:151},WAVE_AMPLITUDE:{min:.1,max:.2},ORBITAL_SPEED:{min:.2,max:.8},PORTAL_COUNT:{min:21,max:60},ENERGY_BEAM_COUNT:{min:2,max:8},PARTICLE_COUNT:{min:201,max:801}};class V{group;plasmaRings=[];energyWaves=[];dimensionalPortals=[];energyBeams=[];energyParticles;particleOrbitData=[];spaceDistortion;params;rng;planetRadius;cosmicOffset;constructor(i,a={}){this.planetRadius=i;const t=a.seed||Math.floor(Math.random()*1e6);this.rng=new T(t),this.params={color:a.color||[.2,.6,1],ringCount:a.ringCount||Math.floor(this.rng.random()*(c.RING_COUNT.max-c.RING_COUNT.min)+c.RING_COUNT.min),ringDistance:a.ringDistance||this.rng.random()*(c.RING_DISTANCE.max-c.RING_DISTANCE.min)+c.RING_DISTANCE.min,waveCount:a.waveCount||Math.floor(this.rng.random()*(c.WAVE_COUNT.max-c.WAVE_COUNT.min)+c.WAVE_COUNT.min),waveAmplitude:a.waveAmplitude||this.rng.random()*(c.WAVE_AMPLITUDE.max-c.WAVE_AMPLITUDE.min)+c.WAVE_AMPLITUDE.min,orbitalSpeed:a.orbitalSpeed||this.rng.random()*(c.ORBITAL_SPEED.max-c.ORBITAL_SPEED.min)+c.ORBITAL_SPEED.min,portalCount:a.portalCount||Math.floor(this.rng.random()*(c.PORTAL_COUNT.max-c.PORTAL_COUNT.min)+c.PORTAL_COUNT.min),energyBeamCount:a.energyBeamCount||Math.floor(this.rng.random()*(c.ENERGY_BEAM_COUNT.max-c.ENERGY_BEAM_COUNT.min)+c.ENERGY_BEAM_COUNT.min),particleCount:a.particleCount||Math.floor(this.rng.random()*(c.PARTICLE_COUNT.max-c.PARTICLE_COUNT.min)+c.PARTICLE_COUNT.min),cosmicOriginTime:a.cosmicOriginTime||E,seed:t},this.cosmicOffset=t%100*.1,this.group=new U,this.createPlasmaRings(),this.createEnergyWaves(),this.createDimensionalPortals(),this.createEnergyBeams(),this.createEnergyParticles(),this.createSpaceDistortion()}createPlasmaRings(){const i=this.params.ringCount,a=this.planetRadius+this.params.ringDistance;for(let t=0;t<i;t++){const n=a+t*.8,e=this.rng.random()*Math.PI*.5,s=this.rng.random()*Math.PI*2,o=(this.rng.random()-.5)*Math.PI*.3,r=new G(this.planetRadius*.2+t*.1,this.planetRadius*.03,8,32),l=new p({uniforms:{time:{value:0},color:{value:new u(this.params.color[0],this.params.color[1],this.params.color[2])},opacity:{value:.6-t*.08},ringIndex:{value:t},pulseSpeed:{value:2+this.rng.random()*2}},vertexShader:`
          uniform float time;
