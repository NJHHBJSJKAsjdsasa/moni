import{B as S,h as A,e as I,i as W,V as w,j as E,G as F,F as O,L as b}from"./atlas_kKpPRC9Ko2x1xhYFTYWg4.js";let z=0;function G(s){z=s}function D(s){return Date.now()/1e3-s+z}function j(s,t=1,i=0){const e=D(s);return i+e*t}const T=51408e4;class C{seed;constructor(t){this.seed=t}next(){return this.seed=this.seed*16807%2147483647,(this.seed-1)/2147483646}uniform(t,i){return t+(i-t)*this.next()}choice(t){return t[Math.floor(this.next()*t.length)]}}class B{ringSystem;concentricLines;material;linesMaterial;params;planetRadius;actualInnerRadius=0;actualOuterRadius=0;scale=1;tiltAngle=0;constructor(t,i){this.planetRadius=t,this.params=i,this.createRingSystemFromAPI(i),this.createConcentricLines()}weightedChoice(t,i,e){const n=e.uniform(0,1);let a=0;for(let l=0;l<i.length;l++)if(a+=i[l],n<a)return t[l];return t[t.length-1]}createRingSystemFromAPI(t){const{inner_radius:i,outer_radius:e,tilt_factor:n,planet_radius:a,shape_seed:l,num_particles_per_band:p,bands:c}=t;if(!c||c.length===0)return;this.scale=this.planetRadius/(a||200),this.tiltAngle=Math.asin((n||.2)*.5),this.actualInnerRadius=(i||200)*this.scale,this.actualOuterRadius=(e||400)*this.scale;const r=new C(l||12345),v=Math.floor((p||800)*1.5),y=c.length*v*2,u=new S,h=new Float32Array(y*3),d=new Float32Array(y*3),P=new Float32Array(y),L=[.4,.3,.2,.1],M=[.5,1,1.5,2];let o=0;for(const _ of c){for(let R=0;R<v;R++){const f=r.uniform(Math.PI,2*Math.PI),m=r.uniform(_.inner,_.outer)*this.scale,g=r.uniform(20,50)/255,x=this.weightedChoice(M,L,r);h[o*3]=m*Math.cos(f),h[o*3+1]=m*Math.sin(this.tiltAngle)*Math.sin(f)-this.planetRadius*.07,h[o*3+2]=m*Math.cos(this.tiltAngle)*Math.sin(f),d[o*3]=g,d[o*3+1]=g,d[o*3+2]=g,P[o]=x*.25,o++}for(let R=0;R<v;R++){const f=r.uniform(0,Math.PI),m=r.uniform(_.inner,_.outer)*this.scale,g=r.uniform(20,50)/255,x=this.weightedChoice(M,L,r);h[o*3]=m*Math.cos(f),h[o*3+1]=m*Math.sin(this.tiltAngle)*Math.sin(f)-this.planetRadius*.07,h[o*3+2]=m*Math.cos(this.tiltAngle)*Math.sin(f),d[o*3]=g,d[o*3+1]=g,d[o*3+2]=g,P[o]=x*.25,o++}}u.setAttribute("position",new A(h,3)),u.setAttribute("color",new A(d,3)),u.setAttribute("size",new A(P,1)),this.material=new I({uniforms:{brightness:{value:4.4},lightDirection:{value:new w(1,0,0)},ambientLight:{value:.15},planetRadius:{value:this.planetRadius},planetWorldPos:{value:new w(0,0,0)}},vertexShader:`
      ...
      `,transparent:!0,depthWrite:!1,blending:W});for(let n=0;n<i;n++){const a=new C(t+n*31);if(a.uniform(0,1)<.1)continue;const l=n/(i-1),p=this.actualInnerRadius+(this.actualOuterRadius-this.actualInnerRadius)*l,c=[];for(let u=0;u<=e;u++){const h=u/e*Math.PI*2,d=p*Math.cos(h),P=p*Math.sin(h),L=P*Math.sin(this.tiltAngle)-this.planetRadius*.07,M=P*Math.cos(this.tiltAngle);c.push(d,L,M)}const r=new S;r.setAttribute("position",new O(c,3));const v=this.linesMaterial.clone();v.uniforms.intensity={value:a.uniform(.4,1)};const y=new b(r,v);this.concentricLines.add(y)}}addToScene(t,i){if(!this.ringSystem)return;const e=i||new w(0,0,0);this.ringSystem.position.copy(e),this.material?.uniforms.planetWorldPos&&this.material.uniforms.planetWorldPos.value.copy(e),this.concentricLines&&(this.concentricLines.position.copy(e),this.concentricLines.children.forEach(n=>{if(n instanceof b){const a=n.material;a.uniforms?.planetWorldPos&&a.uniforms.planetWorldPos.value.copy(e)}})),t.add(this.ringSystem),this.concentricLines&&t.add(this.concentricLines)}update(t,i){if(!this.ringSystem||!i)return;const e=i.rotation_period_seconds||86400,n=i.cosmicOriginTime||T,a=i.initialAngleRotation||0,l=D(n),p=2*Math.PI/e,c=(a+l*p)%(2*Math.PI);this.ringSystem.rotation.y=c,this.concentricLines&&(this.concentricLines.rotation.y=c)}getObject3D(){return this.ringSystem}updateLightDirection(t){const i=t.clone().normalize();this.material?.uniforms.lightDirection&&this.material.uniforms.lightDirection.value.copy(i),this.concentricLines&&this.concentricLines.children.forEach(e=>{if(e instanceof b){const n=e.material;n.uniforms?.lightDirection&&n.uniforms.lightDirection.value.copy(i)}})}updateFromThreeLight(t){const i=new w().subVectors(t.position,t.target.position).normalize();this.material?.uniforms.lightDirection&&this.material.uniforms.lightDirection.value.copy(i),this.concentricLines&&this.concentricLines.children.forEach(e=>{if(e instanceof b){const n=e.material;n.uniforms?.lightDirection&&n.uniforms.lightDirection.value.copy(i)}})}dispose(){this.material&&this.material.dispose(),this.linesMaterial&&this.linesMaterial.dispose(),this.concentricLines&&this.concentricLines.children.forEach(t=>{t instanceof b&&(t.geometry.dispose(),t.material.dispose())})}}function N(s,t){const i={inner_radius:s.inner_radius,outer_radius:s.outer_radius,tilt_factor:s.tilt_factor,planet_radius:s.planet_radius,shape_seed:s.shape_seed,num_particles_per_band:s.num_particles_per_band,bands:s.bands};return new B(t,i)}export{T as D,B as R,D as a,N as c,j as g,G as s};
