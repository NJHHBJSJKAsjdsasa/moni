import{S as w}from"./atlas_CyINvvA9z94U_VTNKIak4.js";import{cp as A,C as D,V as m,au as O,g as x,i as L,j as H,cD as E,D as b,m as I,cj as N,cv as z}from"./atlas_CvLHNd-mv9CFAycOCNrF6.js";const l={HOLE_COUNT:{min:22,max:36},HOLE_RADIUS:{min:.03,max:.1},HOLE_DEPTH:{min:.02,max:.12},ROUGHNESS:{min:.4,max:.8},COLOR_VARIATION:{min:.2,max:.5}};class U{group;planetRadius;rng;holesData=[];coneMeshes=[];holeMask;holeColor;constructor(o,e={},t){this.group=new A,this.planetRadius=o;const i=t||12345;this.rng=new w(i),this.holeColor=e.holeColor instanceof D?e.holeColor:new D(e.holeColor||"#000000"),this.holesData=this.generateProceduralHoles(),this.holesData.length>0&&this.createHoles()}generateProceduralHoles(){const o=[],e=this.rng.randint(l.HOLE_COUNT.min,l.HOLE_COUNT.max);for(let t=0;t<e;t++){const i=this.rng.random()*2*Math.PI,a=Math.acos(this.rng.random()*2-1),h=[Math.sin(a)*Math.cos(i),Math.sin(a)*Math.sin(i),Math.cos(a)];o.push({position_3d:h,radius:this.rng.uniform(l.HOLE_RADIUS.min,l.HOLE_RADIUS.max),depth:this.rng.uniform(l.HOLE_DEPTH.min,l.HOLE_DEPTH.max),roughness:this.rng.uniform(l.ROUGHNESS.min,l.ROUGHNESS.max),color_variation:this.rng.uniform(l.COLOR_VARIATION.min,l.COLOR_VARIATION.max)})}return o}createPlanetHoleShader(o){const t=Math.min(this.holesData.length,64),i=`
