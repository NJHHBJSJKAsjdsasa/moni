import{S as C}from"./atlas_CGEzHqiP6UrmR4JlMl8bd.js";import{D as u,a as D}from"./atlas_Be-3E54fbq6i1G6LOfhmR.js";import{cp as U,V as P,cx as L,cy as G,av as f,au as F,C as S,D as W,bE as X,m as Y}from"./atlas_BkM-4aqZ6cV-vWVlt631_.js";const n={RING_COUNT:{min:16,max:256},EXPANSION_SPEED:{min:1.5,max:5},WAVE_INTENSITY:{min:.6,max:2},MAX_RADIUS_MULTIPLIER:{min:1.5,max:10},GLOW_INTENSITY:{min:.5,max:2},TIME_SPEED:{min:.8,max:1.3}};class V{group;concentricRings=[];params;rng;planetRadius;cosmicOffset;constructor(t,e={}){this.planetRadius=t;const i=e.seed||Math.floor(Math.random()*1e6);this.rng=new C(i),this.params={color:e.color||[.3,1,.2],ringCount:e.ringCount||Math.floor(this.rng.random()*(n.RING_COUNT.max-n.RING_COUNT.min)+n.RING_COUNT.min),expansionSpeed:e.expansionSpeed||this.rng.random()*(n.EXPANSION_SPEED.max-n.EXPANSION_SPEED.min)+n.EXPANSION_SPEED.min,waveIntensity:e.waveIntensity||this.rng.random()*(n.WAVE_INTENSITY.max-n.WAVE_INTENSITY.min)+n.WAVE_INTENSITY.min,maxRadius:e.maxRadius||t*(this.rng.random()*(n.MAX_RADIUS_MULTIPLIER.max-n.MAX_RADIUS_MULTIPLIER.min)+n.MAX_RADIUS_MULTIPLIER.min),glowIntensity:e.glowIntensity||this.rng.random()*(n.GLOW_INTENSITY.max-n.GLOW_INTENSITY.min)+n.GLOW_INTENSITY.min,cosmicOriginTime:e.cosmicOriginTime||u,timeSpeed:e.timeSpeed||this.rng.random()*(n.TIME_SPEED.max-n.TIME_SPEED.min)+n.TIME_SPEED.min,seed:i},this.cosmicOffset=i%100*.1,this.group=new U,this.createConcentricRings()}createConcentricRings(){const t=this.params.ringCount,e=this.params.maxRadius;for(let i=0;i<t;i++){const s=i/(t-1),r=this.planetRadius*1.02+(e-this.planetRadius*1.02)*Math.pow(s,.8),c=64,g=[];for(let a=0;a<c;a++){const l=a/c*Math.PI*2,w=Math.cos(l)*r,O=Math.sin(l)*r,M=this.planetRadius*.005,A=Math.sin(l*7+i*2)*.5*.01*this.planetRadius,b=M+A;g.push(new P(w,b,O))}const E=new L(g,!0),R=this.planetRadius*.006,y=4,x=c,o=new G(E,x,R,y,!0),T=o.attributes.position.array,_=o.attributes.uv.array,m=T.length/3,h=new Float32Array(m),v=new Float32Array(m),I=new Float32Array(m);for(let a=0;a<m;a++){const l=_[a*2];h[a]=i*.6+l*Math.PI*4,v[a]=(r-this.planetRadius*1.02)/(e-this.planetRadius*1.02),I[a]=(i*7+a*3)%100/100}o.setAttribute("phase",new f(h,1)),o.setAttribute("distance",new f(v,1)),o.setAttribute("randomOffset",new f(I,1));const N=new F({transparent:!0,depthWrite:!1,depthTest:!0,blending:X,side:W,uniforms:{time:{value:0},color:{value:new S(this.params.color[0],this.params.color[1],this.params.color[2])},expansionSpeed:{value:this.params.expansionSpeed},waveIntensity:{value:this.params.waveIntensity},glowIntensity:{value:this.params.glowIntensity},ringIndex:{value:i},totalRings:{value:t}},vertexShader:`
        ...
        `}),d=new Y(o,N);d.renderOrder=1e3+i,this.concentricRings.push(d),this.group.add(d)}}update(t){const s=(D(this.params.cosmicOriginTime||u)+this.cosmicOffset)*(this.params.timeSpeed||1)%1e4;this.concentricRings.forEach(r=>{const c=r.material;c.uniforms.time.value=s}),this.group.rotation.y=s*.05}getObject3D(){return this.group}addToScene(t,e){e&&this.group.position.copy(e),t.add(this.group)}removeFromScene(t){t.remove(this.group)}dispose(){this.concentricRings.forEach(t=>{t.geometry.dispose(),t.material.dispose()}),this.group.clear()}setEnabled(t){this.group.visible=t}updateParams(t){if(Object.assign(this.params,t),t.color){const e=new S(t.color[0],t.color[1],t.color[2]);this.concentricRings.forEach(i=>{const s=i.material;s.uniforms.color.value=e})}t.expansionSpeed!==void 0&&this.concentricRings.forEach(e=>{const i=e.material;i.uniforms.expansionSpeed.value=t.expansionSpeed}),t.waveIntensity!==void 0&&this.concentricRings.forEach(e=>{const i=e.material;i.uniforms.waveIntensity.value=t.waveIntensity}),t.glowIntensity!==void 0&&this.concentricRings.forEach(e=>{const i=e.material;i.uniforms.glowIntensity.value=t.glowIntensity})}}function H(p,t,e){const s={seed:(e||Math.floor(Math.random()*1e6))+42424,color:t.color||[.3,1,.2],cosmicOriginTime:t?.timing?.cosmic_origin_time||t?.cosmicOriginTime||u};return new V(p,s)}export{V as R,H as c};
