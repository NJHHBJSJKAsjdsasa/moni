import{S}from"./atlas_CGEzHqiP6UrmR4JlMl8bd.js";import{D as H,a as Y}from"./atlas_Be-3E54fbq6i1G6LOfhmR.js";import{cp as X,bO as Z,c4 as k,r as q,I as _,C as F,V as m,cF as J,au as p,g as K,M as O,m as A,i as Q,av as N,bE as L,co as ee,B as te,cv as W}from"./atlas_BkM-4aqZ6cV-vWVlt631_.js";const h={CRYSTAL_COUNT:{min:100,max:400},SIZE:{min:.8,max:1.2},TRANSMISSION:{min:0,max:0},IOR:{min:2.4,max:2.1},ROUGHNESS:{min:0,max:.01},GLOW_INTENSITY:{min:.1,max:2.1},HEIGHT:{min:.01,max:.09}};class ie{constructor(e,t={},a,o){this.planetRadius=e,this.params=t,this.seededRng=a,this.starField=o,this.crystallineGroup=new X,this.crystallineGroup.name="CrystallineSurface",this.animationSpeed=t.timeSpeed||.05,this.createStarfieldCubemap(),this.generateCrystallineFormations(),this.starField&&setTimeout(()=>{this.starField&&this.starField.getObject3D&&this.starField.getObject3D()&&this.createStarFieldReflections()},0)}crystallineGroup;crystallineFormations=[];glowMeshes=[];animationSpeed;envMap=null;starFieldReflections=null;starFieldMaterial=null;crystallineData=[];createStarfieldCubemap(){try{const t=[];for(let a=0;a<6;a++){const o=document.createElement("canvas");o.width=512,o.height=512;const i=o.getContext("2d");if(!i)continue;const n=i.createRadialGradient(512/2,512/2,0,512/2,512/2,512/2);n.addColorStop(0,"#1a1a2e"),n.addColorStop(.5,"#16213e"),n.addColorStop(1,"#0f0f23"),i.fillStyle=n,i.fillRect(0,0,512,512);const s=new S((this.params.seed||42)+a*1e3),d=200+Math.floor(s.random()*100);for(let v=0;v<d;v++){const c=s.random()*512,l=s.random()*512,r=.3+s.random()*.4,g=s.random()*2+.5,u=s.random();u<.4?i.fillStyle=`rgba(${120+60*r}, ${140+80*r}, 255, ${r})`:u<.7?i.fillStyle=`rgba(${200+55*r}, ${200+55*r}, ${200+55*r}, ${r})`:i.fillStyle=`rgba(255, ${180+60*r}, ${120+60*r}, ${r})`,i.beginPath(),i.arc(c,l,g,0,Math.PI*2),i.fill(),r>.6&&s.random()<.1&&(i.fillStyle=`rgba(255, 255, 255, ${r*.2})`,i.beginPath(),i.arc(c,l,g*2,0,Math.PI*2),i.fill())}t.push(o)}t.length===6&&(this.envMap=new Z(t),this.envMap.needsUpdate=!0,this.envMap.mapping=k,this.envMap.format=q,this.envMap.generateMipmaps=!1,this.envMap.minFilter=_,this.envMap.magFilter=_)}catch{}}generateCrystallineFormations(){this.crystallineData=this.params.crystallinePatches||this.generateProceduralCrystals(),this.crystallineData.forEach((e,t)=>{this.createCrystalFormation(e,t)})}generateProceduralCrystals(){const e=this.seededRng||new S(this.params.seed||42),t=e.randint(h.CRYSTAL_COUNT.min,h.CRYSTAL_COUNT.max),a=[],o=this.params.baseColor instanceof F?this.params.baseColor:new F(this.params.baseColor||52479);for(let i=0;i<t;i++){const n=e.random()*Math.PI*2,s=Math.acos(e.random()*2-1),d=[Math.sin(s)*Math.cos(n),Math.sin(s)*Math.sin(n),Math.cos(s)],v=.8+e.random()*.2,c=o.clone().multiplyScalar(v);a.push({position_3d:d,size:e.uniform(h.SIZE.min,h.SIZE.max),color:[c.r,c.g,c.b,.9],height:e.uniform(h.HEIGHT.min,h.HEIGHT.max),sides:6+Math.floor(e.random()*6),transmission:e.uniform(h.TRANSMISSION.min,h.TRANSMISSION.max),ior:e.uniform(h.IOR.min,h.IOR.max),roughness:e.uniform(h.ROUGHNESS.min,h.ROUGHNESS.max),glowIntensity:e.uniform(h.GLOW_INTENSITY.min,h.GLOW_INTENSITY.max)})}return a}createCrystalFormation(e,t){const a=new m(e.position_3d[0],e.position_3d[1],e.position_3d[2]).normalize(),o=e.size*this.planetRadius*.5,i=e.height*this.planetRadius,n=e.sides,s=4,d=new J(o*.8,o,i,n,s),v=new p({uniforms:{color:{value:new F(e.color[0],e.color[1],e.color[2])},envMap:{value:this.envMap},roughness:{value:e.roughness},ior:{value:e.ior},transmission:{value:.2},lightPosition:{value:new m(0,0,0)},lightDirection:{value:new m(1,1,1).normalize()}},vertexShader:`
      ...
      `,transparent:!0,side:K}),c=a.clone().multiplyScalar(this.planetRadius),l=c.clone().normalize(),r=new m,g=new m;Math.abs(l.y)<.99?r.crossVectors(l,new m(0,1,0)).normalize():r.crossVectors(l,new m(1,0,0)).normalize(),g.crossVectors(l,r).normalize();const u=t*137.5%360*(Math.PI/180),x=r.clone().multiplyScalar(Math.cos(u)).add(g.clone().multiplyScalar(Math.sin(u))),V=new m().crossVectors(l,x).normalize(),y=new O;y.makeRotationX(Math.PI/2),d.applyMatrix4(y);const P=new O;P.makeBasis(x,V,l);const f=d.attributes.position,z=new m;d.applyMatrix4(P);for(let w=0;w<f.count;w++){z.fromBufferAttribute(f,w);const b=z.clone().add(c),R=b.length(),E=b.clone().normalize(),T=this.planetRadius+(R-this.planetRadius)*.5,M=E.multiplyScalar(T).sub(c);f.setXYZ(w,M.x,M.y,M.z)}f.needsUpdate=!0,d.computeVertexNormals(),this.applyProceduralNoise(d,e.size*.3),d.translate(c.x,c.y,c.z);const C=new A(d,v);this.createInnerGlow(C,e,t),this.crystallineFormations.push(C),this.crystallineGroup.add(C)}applyProceduralNoise(e,t){const a=e.getAttribute("position"),o=new m;for(let i=0;i<a.count;i++){o.fromBufferAttribute(a,i);const s=this.simpleNoise(o.x*10,o.y*10,o.z*10)*t*.02;o.multiplyScalar(1+s),a.setXYZ(i,o.x,o.y,o.z)}e.setAttribute("position",a),e.computeVertexNormals()}simpleNoise(e,t,a){return(Math.sin(e*.1)*Math.cos(t*.1)*Math.sin(a*.1)+Math.sin(e*.05)*Math.cos(t*.05)*.5+Math.sin(e*.2)*Math.sin(a*.15)*.3)*.5}createStarFieldReflections(){if(!this.starField||!this.starField.getObject3D)return;const e=this.starField.getObject3D();if(!e.geometry)return;const t=e.geometry.attributes.position,a=e.geometry.attributes.size,o=e.geometry.attributes.brightness;if(!t||!a||!o)return;const i=[];if(this.crystallineData.forEach((l,r)=>{const u=new m(l.position_3d[0],l.position_3d[1],l.position_3d[2]).normalize().clone().multiplyScalar(this.planetRadius),x=u.clone().normalize();this.generateDeterministicFaceNormals(l,r).forEach((y,P)=>{for(let f=0;f<t.count;f++){const z=t.getX(f),C=t.getY(f),w=t.getZ(f),b=o.getX(f),R=a.getX(f);if(b<.6)continue;const I=new m(z,C,w).clone().normalize().clone().negate(),M=I.dot(y),B=I.clone().sub(y.clone().multiplyScalar(2*M)),D=Math.abs(B.dot(x)),j=r*1e4+P*1e3+f,U=new S(j+(this.params.seed||42)).random()<.4;if(D>.3&&M<-.1&&U){const $=u.clone().add(y.clone().multiplyScalar(.001)).clone().normalize().multiplyScalar(this.planetRadius*1.001);i.push({position:$,size:R*.4*D,brightness:b*.5*D,crystalIndex:r})}}})}),i.length===0)return;const n=new Float32Array(i.length*3),s=new Float32Array(i.length),d=new Float32Array(i.length),v=new Float32Array(i.length);i.forEach((l,r)=>{n[r*3]=l.position.x,n[r*3+1]=l.position.y,n[r*3+2]=l.position.z,s[r]=l.size,d[r]=l.brightness,v[r]=l.crystalIndex});const c=new Q;c.setAttribute("position",new N(n,3)),c.setAttribute("size",new N(s,1)),c.setAttribute("brightness",new N(d,1)),c.setAttribute("crystalIndex",new N(v,1)),this.starFieldMaterial=new p({uniforms:{planetRadius:{value:this.planetRadius},lightDirection:{value:new m(1,1,1).normalize()}},vertexShader:`
      ...
      `,transparent:!0,blending:L,depthWrite:!1}),this.starFieldReflections=new ee(c,this.starFieldMaterial),this.crystallineGroup.add(this.starFieldReflections)}generateDeterministicFaceNormals(e,t){const a=new S((this.params.seed||42)+t*1e3),o=[],i=e.sides||6,n=Math.min(i,8),s=new m(e.position_3d[0],e.position_3d[1],e.position_3d[2]).normalize(),d=new m,v=new m;Math.abs(s.y)<.99?d.crossVectors(s,new m(0,1,0)).normalize():d.crossVectors(s,new m(1,0,0)).normalize(),v.crossVectors(s,d).normalize();for(let c=0;c<n;c++){const l=c/n*Math.PI*2,r=a.uniform(-Math.PI*.3,Math.PI*.3),g=new m().addScaledVector(d,Math.cos(l)).addScaledVector(v,Math.sin(l)).addScaledVector(s,Math.sin(r)).normalize();o.push(g)}return o}createInnerGlow(e,t,a){const o=e.geometry.clone(),i=new p({uniforms:{time:{value:0},glowColor:{value:new F(t.color[0],t.color[1],t.color[2])},glowIntensity:{value:t.glowIntensity||.5},lightPosition:{value:new m(0,0,0)},lightDirection:{value:new m(1,1,1).normalize()}},vertexShader:`
        varying vec3 vNormal;
