import{S as m}from"./atlas_CGEzHqiP6UrmR4JlMl8bd.js";import{C as r,cr as f,au as h,bE as p,D as v,m as C}from"./atlas_BkM-4aqZ6cV-vWVlt631_.js";const e={CRACK_DENSITY:{min:.3,max:.6},CRACK_DEPTH:{min:.5,max:.9},CRACK_COMPLEXITY:{min:3,max:5},CRACK_SCALE:{min:3,max:8},CRACK_SHARPNESS:{min:.7,max:1},CRACK_OPACITY:{min:.6,max:.9},INTERNAL_REFLECTIONS:{min:.5,max:.9},ANIMATION_SPEED:{min:.05,max:.15}};class u{crackMesh;material;params;constructor(a={}){const o=a.seed!==void 0?a.seed:123456,t=new m(o+1e4),i=t.uniform(0,360),s=t.uniform(.3,.8),c=t.uniform(.7,1),l=new r().setHSL(i/360,s,c);this.params={crackDensity:t.uniform(e.CRACK_DENSITY.min,e.CRACK_DENSITY.max),crackDepth:t.uniform(e.CRACK_DEPTH.min,e.CRACK_DEPTH.max),crackComplexity:Math.floor(t.uniform(e.CRACK_COMPLEXITY.min,e.CRACK_COMPLEXITY.max)),crackScale:t.uniform(e.CRACK_SCALE.min,e.CRACK_SCALE.max),crackSharpness:t.uniform(e.CRACK_SHARPNESS.min,e.CRACK_SHARPNESS.max),crackColor:l,crackOpacity:t.uniform(e.CRACK_OPACITY.min,e.CRACK_OPACITY.max),internalReflections:t.uniform(e.INTERNAL_REFLECTIONS.min,e.INTERNAL_REFLECTIONS.max),animationSpeed:t.uniform(e.ANIMATION_SPEED.min,e.ANIMATION_SPEED.max),seed:o,radius:a.radius||2.5,lineDrawPercentage:a.lineDrawPercentage!==void 0?a.lineDrawPercentage:.66};const d=new f(this.params.radius*1.02,32,32);this.material=new h({vertexShader:`
      ...
      `,uniforms:{time:{value:0},crackColor:{value:this.params.crackColor||new r(65535)},crackIntensity:{value:(this.params.crackOpacity||.7)*2},crackGlow:{value:(this.params.internalReflections||.7)*3},planetSeed:{value:this.params.seed||123456},lineDrawPercentage:{value:this.params.lineDrawPercentage||.66},rotationAngle:{value:0}},transparent:!0,depthWrite:!1,side:v,blending:p}),this.crackMesh=new C(d,this.material)}addToScene(a,o){this.crackMesh.position.copy(o),this.crackMesh.visible=!0,a.add(this.crackMesh)}removeFromScene(a){a.remove(this.crackMesh)}update(a,o){this.material.uniforms.time&&(this.material.uniforms.time.value+=a*this.params.animationSpeed),o!==void 0&&this.material.uniforms.rotationAngle&&(this.material.uniforms.rotationAngle.value=o)}updateFromThreeLight(a){}getObject3D(){return this.crackMesh}dispose(){this.crackMesh&&(this.crackMesh.geometry.dispose(),this.material.dispose())}}function A(n,a,o){const i=o?(o+5e3)%1e6:Math.floor(Math.random()*1e6);return new u({radius:a,seed:i,lineDrawPercentage:.66})}export{u as T,A as c};
